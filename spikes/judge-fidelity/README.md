...
Story 1.26 successor planning now requires a canonical `oddspark.judge-historical-spend-closure/v1` record created with `run.mjs close-history`. The record retains the original completed receipt in place, binds its exact bytes plus the completed evidence, Markdown, qualification, and completion marker, and records 42 cumulative historical calls. The known priced-model cost is exact; the unpriced selected 8B model remains explicitly unpriced under the retained conservative historical cap. Closure is validated before plan creation and, on the public live boundary, before approval loading, adapter diagnostics, allowance reservation, or runner/provider work. A successor plan must have a distinct run id and binds the closure ref; its fresh allowance adds to rather than resets historical accounting. The live path uses a distinct successor receipt and permits one runner invocation with zero retries or replacements only after a fresh exact approval binds the full successor plan.
