...
async function approvedRecovery({ approvalRunId = "approval-run-fixture", now = new Date("2026-08-18T12:00:00.000Z"), planType = "paid", remainingFreeNeurons = 20_000 } = {}) {
  ...
  assert.equal(validateRecoveryPlan(current.recoveryPlan).valid, true);
...
test("free-plan headroom rejects insufficient allowance and accepts the exact conservative maximum", async () => {
  ...
  assert.equal(validateRecoveryPlan(exactSetup.recoveryPlan).valid, true);
});
...
test("the explicit qualification verifier independently verifies a sibling evidence artifact", async () => {
  const now = new Date();
  const setup = await approvedRecovery({ approvalRunId: "explicit-qualification-run", now });
  const retained = await operationalEvidence({ runId: "explicit-qualification-run", runStartedAt: now.toISOString() });
  const evidenceFile = "explicit-evidence-v2.json";
  const bytes = Buffer.from(`${JSON.stringify(retained.evidence, null, 2)}\n`);
  const dependencies = { currentSourceIdentity: async () => retained.sources, currentRuntimeIdentity: async () => retained.runtime, executeFixtures: async () => retained.fixtureResult };
  const bundle = await buildQualificationBundle({ plan: setup.recoveryPlan, approval: setup.approval, evidence: retained.evidence, evidence_file: evidenceFile, evidence_bytes: bytes, now }, dependencies);
  const directory = await mkdtemp(path.join(tmpdir(), "oddspark-qualification-verifier-"));
  const evidencePath = path.join(directory, evidenceFile);
  const bundlePath = path.join(directory, "explicit-qualification.json");
...
test("a stale dead-process lock is recoverable only with a valid zero-call reservation", async () => {
  ...
  await writeFile(lockPath, `${JSON.stringify({ pid: 2_147_483_647, created_at: timestamp })}\n`);
  const old = new Date(Date.now() - 10 * 60 * 1000);
  await utimes(lockPath, old, old);
  const lock = await acquireRecoveryLock(directory, { now: new Date(), stale_ms: 5 * 60 * 1000 });
...
test("invalid, partial, tampered, and unverified-ref artifacts never reopen or embellish the allowance", async () => {
  ...
  assert.match((await findPriorOperationalRecovery(zeroDirectory, zeroDependencies)).blocking_reason, /failed complete verification/);
});
