...
export const PLAN_VERSION = "oddspark.generation-qualification-plan/v1";
export const APPROVAL_VERSION = "oddspark.generation-qualification-approval/v1";
export const MANIFEST_VERSION = "STRUCT-GENERATION/v1";
export const QUALIFICATION_DOMAIN = "oddspark-struct-generation-qualification/v1";
export const PLAN_DOMAIN = "oddspark-generation-qualification-plan/v1";
...
export const PRICING = Object.freeze({ as_of: "2026-08-18", source: "https://developers.cloudflare.com/workers-ai/platform/pricing/", currency: "USD", input_per_million: { "@cf/openai/gpt-oss-120b": 0.35, "@cf/openai/gpt-oss-20b": 0.20 }, output_per_million: { "@cf/openai/gpt-oss-120b": 0.75, "@cf/openai/gpt-oss-20b": 0.30 } });
const exact = (value, keys) => value && typeof value === "object" && !Array.isArray(value) && Object.keys(value).length === keys.length && keys.every((key) => Object.hasOwn(value, key));

export function estimate(requests) {
  const roles = requests.map((request) => { const input_tokens_upper_bound = Buffer.byteLength(stableStringify(request.body), "utf8"); const calls = 21; const input_usd = calls * input_tokens_upper_bound * PRICING.input_per_million[request.resolved_model] / 1e6; const output_usd = calls * PARAMETERS.max_tokens * PRICING.output_per_million[request.resolved_model] / 1e6; return { role: request.role, resolved_model: request.resolved_model, calls, input_tokens_upper_bound, max_output_tokens_per_call: PARAMETERS.max_tokens, input_usd, output_usd, maximum_usd: input_usd + output_usd }; }); return { pricing: PRICING, roles, call_cap: CALL_CAP, maximum_usd: roles.reduce((sum, role) => sum + role.maximum_usd, 0) };
}
...
export function costForUsage(model, usage) {
  ...
  if (!exact(usage, ["input_tokens", "output_tokens"]) || ![usage.input_tokens, usage.output_tokens].every((value) => Number.isSafeInteger(value) && value >= 0) || !PRICING.input_per_million[model]) return undefined;
  return usage.input_tokens * PRICING.input_per_million[model] / 1e6 + usage.output_tokens * PRICING.output_per_million[model] / 1e6;
}
