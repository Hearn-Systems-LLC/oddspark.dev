...
import { buildEvidence, deepFreeze, deriveEvidenceRef } from "./brief-contracts.mjs";
import { projectLocalPrior, verifyLocalPriors } from "./local-priors.mjs";
...
function snapshotRequest(input, { includePaths = false } = {}) {
...
export function assembleLocalEvidence(input, dependencies = {}) {
  ...
  const seams = snapshotDependencies(dependencies);
...
export async function assembleLocalEvidenceFromFiles(input, dependencies = {}) {
  const request = snapshotRequest(input, { includePaths: true });
  const seams = snapshotDependencies(dependencies);
