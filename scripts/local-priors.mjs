...
export function canonicalJson(value) {
  ...
  function encode(current) {
    ...
    if (["string", "boolean"].includes(typeof current)) return JSON.stringify(current);
    if (typeof current === "number") {
      if (!Number.isFinite(current)) throw new NonJsonValueError("canonical JSON rejects non-finite numbers");
      return JSON.stringify(current);
    }
    if (["undefined", "bigint", "function", "symbol"].includes(typeof current)) throw new NonJsonValueError(`canonical JSON rejects ${typeof current}`);
    if (!Array.isArray(current) && !isPlainObject(current)) throw new NonJsonValueError("canonical JSON rejects non-plain objects");
    if (ancestors.has(current)) throw new NonJsonValueError("canonical JSON rejects circular values");
    ancestors.add(current);
    try {
      if (Array.isArray(current)) {
        for (let index = 0; index < current.length; index += 1) if (!Object.hasOwn(current, index)) throw new NonJsonValueError("canonical JSON rejects sparse arrays");
...
export function domainHash(domain, value) {
  return createHash("sha256").update(`oddspark.local-priors.v1\0${domain}\0${canonicalJson(value)}`, "utf8").digest("hex");
}
