...
- 2026-08-23 — Governance record (human renegotiation): a0ed5363 cycle is owner-reviewed immutable history.
  ...
  - Independent-review criterion: Stories 1.12/1.13 re-verify this cycle with `npm run spike:judge:verify` and `npm run spike:judge:qualification:verify` against the restored source bytes. The plan-governance assertion in `test.mjs:1105` is not part of STRUCT-JUDGE identity — it is leftover from before the spent GO cycle existed (DW-6). `planCommand` against the live results directory correctly refuses a further matrix; that refusal is not flipped and no successor matrix is granted.
  - CI exception: `.github/workflows/test.yml` runs `npm run check` minus `spike:judge:self-test` (the restored success branch is 80/81 against the spent GO cycle) and runs the two identity verifiers instead. `package.json` `check` is evidence-pinned and is not edited. Local `npm run check` remains red on that one test until a later cycle drops `test.mjs` from `EVIDENCE_SOURCE_PATHS`.
  - Known-bad state avoided: treating a GO identity as identity-drifted history (the NO-GO predecessor pattern), flipping a byte-pinned source to encode the next review-state change, or making a fourth matrix plannable so a stale test would pass.
