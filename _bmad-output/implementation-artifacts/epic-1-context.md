...
- **Generation role topology (decided 2026-08-22 by Justin, Story 1.11 run l9):** the generation role is qualified primary-only (`@cf/meta/llama-3.3-70b-instruct-fp8-fast`, four consecutive 20/20 GO cycles); the fallback model leg is permanently unwired and the activation manifest carries no generation-fallback identity. Generation exhaustion or failure serves the approved deterministic house Brief (Story 1.13), which is gate-passing by construction.
