...
**Pipes-and-filters, single orchestrator.** The generator is a pipeline of typed stages, each a banner section in the single-file worker. Stages are single-pass: each consumes only the previous stage's output and returns a typed value. The retry loop lives in exactly one place — the strike orchestrator at the existing `buildSpark` / `buildDomainSpark` call sites.
...
| Concern | Convention |
| --- | --- |
| Layout | Single-file `src/worker.js`, banner sections per pipeline stage; constants hoisted; tunables as consts, model IDs as wrangler vars. [ADOPTED] |
