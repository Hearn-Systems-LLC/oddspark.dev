...
- **Rule:** generation and judge use separate role configurations even when they share a provider/model. A structural identity freezes provider, resolved model (after alias/default resolution), request parameters, prompt-template hash, provider-wire-schema hash, adapter hash, candidate-binding version, runtime/version, and timeout/call-policy hash. A semantic identity versions the rubric and golden/anti-golden corpus. The closed `QualificationManifest` is `{version,role,provider,resolved_model,request_parameters,prompt_template_sha256,wire_schema_sha256,adapter_sha256,binding_version,runtime,timeout_policy_sha256,semantic_identity_sha256,fixture_result_sha256,trial_counts,rates,latency_cost,outcome,approval_run_id,tested_source_identity}` and `qualification_ref = sha256("oddspark-qualification/v1\n" + canonical_json(QualificationManifest))`. Before any live qualification, Story 1.2 freezes a reviewed exact Wrangler version, compatibility date, generated bindings, and runtime configuration. Any later change to those inputs invalidates the affected structural evidence and every dependent semantic, full-request, production-verification, launch, and activation record; all affected stages rerun before activation. A completed marker-bound preflight artifact with zero call records and zero durable call-start accounting remains retained but consumes no qualification allowance. Source drift may make it unusable as current qualification evidence without changing that independently verified zero-call fact; this narrow recovery may restore allowance only and may never emit a manifest or ref. Any call-start evidence or ambiguous accounting remains consumed and blocking. Story 1.4 and Story 1.11 are independent structural gates: Story 1.4 qualifies each judge primary/fallback for provider-wire/canonical fidelity, binding, and isolated latency/cost; Story 1.11 separately qualifies typed Candidate output after the frozen transport mapping defined by AD-2. Story 1.18 qualifies semantics and exact predeploy identity; Story 1.19 may implement offline earlier but completes live full-pair latency/cost evidence only after Story 1.14's authoritative commit path exists. Production requires every applicable stage current. Rates never pool; fallbacks never replace failed primary trials. The 2026-08-19 owner MVP review retains Workers AI and authorizes planning for exactly one fresh, separately approved structural qualification cycle using `@cf/meta/llama-3.3-70b-instruct-fp8-fast` as primary and `@cf/meta/llama-3.1-8b-instruct-fast` as fallback for both roles. Generation and judge qualify independently; neither role borrows the other's evidence. The prior gpt-oss NO-GO evidence remains immutable and is not reclassified. The new cycle does not weaken schemas, thresholds, direct-valid accounting, candidate binding, or repair prohibitions. A NO-GO for either required role ends this Workers AI course and requires a new owner architecture decision before any other model, provider, or AI Gateway evaluation. Model selection and architecture approval do not authorize a live call; each exact plan still requires fresh execution approval.
- **Post-review cycle accounting:** the authorized cycle contains exactly one generation matrix and one judge matrix. Each matrix binds both selected model configurations, has its own exact plan, approval, call cap, cost cap, evidence, and completion marker, and consumes its role allowance when its first durable call-start is recorded. A complete marker-bound zero-call preflight remains non-consuming under the rule above. After first call-start, an interruption, incomplete publication, missing terminal record, or ambiguous accounting retains the attempt as `consumed_incomplete`; it emits no configuration or role qualification ref, blocks that role, and requires owner architecture review before any further matrix. It is never converted into a zero-call recovery or silently resumed. Generation evidence cannot consume, restore, or satisfy judge allowance, and judge evidence cannot consume, restore, or satisfy generation allowance. No retry, replacement model, extra matrix, or unused allowance transfer is implied.
- **Role qualification set:** each configuration emits its own `QualificationManifest` and ref only on GO. The closed `RoleQualificationSet {version,role,primary:{resolved_model,qualification_ref:string|null,outcome:"go"|"no-go"},fallback:{resolved_model,qualification_ref:string|null,outcome:"go"|"no-go"},tested_source_identity,cycle_ref,outcome:"go"|"no-go"}` has no unknown fields; each `go` member requires its exact non-null current ref, each `no-go` member requires null, the two members must be the AD-11 selected pair from the same completed role matrix and source identity, and role outcome is `go` iff at least one member is `go`. `role_qualification_ref = sha256("oddspark-role-qualification/v1\n" + canonical_json(RoleQualificationSet))`. A role with two `no-go` members ends the Workers AI course. Partial success never promotes the failed configuration and never borrows or pools its trials; the one passing configuration is the role's sole selectable configuration.
- **Atomic activation:** the only production activation input is one closed canonical `ProductionActivationManifest {version:int,deployed_source_identity:string,generation_ref:string,judge_ref:string,semantic_ref:string,local:{enabled:boolean,full_request_ref:string|null},domain:{enabled:boolean,evidence_ref:string|null,full_request_ref:string|null},house_catalog_ref:string,receiver_ref:string|null,receipt_claim_ref:string|null,outcome:"active"}` with no unknown fields. `generation_ref` and `judge_ref` are the exact current role-level refs of corresponding `RoleQualificationSet` values whose outcome is `go`, never per-configuration refs or refs to `no-go`, incomplete, ambiguous, stale, or superseded evidence. At least one mode is enabled. Local enablement requires its full-request ref; domain enablement requires its Evidence and full-request refs; disabled modes require their mode-specific refs to be null. The shared generation, judge, and semantic refs appear exactly once. The canonical manifest is the single deployment value, and runtime derives `activation_ref = sha256("oddspark-production-activation/v1\n" + canonical_json(ProductionActivationManifest))`; no second deployed hash or individually supplied ref exists. Runtime rejects partial updates, invalid nullability, and stale/failed/mismatched evidence. A missing or invalid manifest disables model roles, stronger receipt copy, and reference-bearing handoff without crashing the public shell; requests consult the authoritative committed/approved house path when COORD and the deployed catalog remain valid, otherwise they return the defined `502`. Existing platform observability exposes only a stable redacted activation reason code and creates no counter, KV key, or COORD record. The manifest is published only after the compatible reader, authoritative coordinator path, new writer, exact semantic/predeploy evidence, and full-request evidence are complete. Rollback preserves a compatible reader and atomically removes or replaces the whole value. Evidence-v2 is a new manifest, never an overwrite of v1. Production selection resolves each role before pair reservation from its current role set: select primary when its member outcome is `go`; otherwise select fallback when its member outcome is `go`; otherwise the role is disabled and the strike uses the authoritative house path. Once a pair starts, roles do not change. If both members are qualified and an invoked primary returns `unavailable`, `circuit_open`, `timeout`, or invalid provider output, the failed call remains consumed; a subsequent new pair may select fallback only if a complete pair and deadline still fit. A matrix `no-go`, semantic rejection, local Candidate rejection, or any other failure class never switches configurations, and the same Candidate is never re-judged.
...
| Generation and judge candidate — primary | `@cf/meta/llama-3.3-70b-instruct-fp8-fast` (subject to independent Story 1.11 generation and Story 1.4 judge qualification) |
| Generation and judge candidate — fallback | `@cf/meta/llama-3.1-8b-instruct-fast` (subject to independent Story 1.11 generation and Story 1.4 judge qualification) |
