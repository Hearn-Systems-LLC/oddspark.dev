...
// UTF-8 byte length via TextEncoder, so the byte-bounded generation
// classifier runs identically in the Worker isolate and in Node.
export function utf8ByteLength(text) {
  return new TextEncoder().encode(text).length;
}

// One canonical domain-separated content hash: every catalog/corpus identity
// (priors, house Briefs, voice corpus, activation) composes this with its own
// fixed namespace instead of keeping a private copy.
export function domainSeparatedHash(namespace, domain, value) {
  return sha256Hex(`${namespace}\0${domain}\0${encodeCanonicalJson(value)}`);
}
...
// The single canonical-JSON algorithm for every module: recursively sorted
// object keys, preserved array order, strict rejection of non-JSON values.
// `encodeCanonicalJson` throws the strict CanonicalJsonError; `canonicalJson`
// rethrows it as a ContractValidationError for the closed-contract builders.
export function encodeCanonicalJson(value) {
  ...
  function encode(current) {
    ...
    if (typeof current === "number") {
      if (!Number.isFinite(current)) throw new CanonicalJsonError("canonical JSON rejects non-finite numbers");
      return JSON.stringify(current);
    }
    if (["undefined", "bigint", "function", "symbol"].includes(typeof current)) throw new CanonicalJsonError(`canonical JSON rejects ${typeof current}`);
    if (!Array.isArray(current) && !isObject(current)) throw new CanonicalJsonError("canonical JSON rejects non-plain objects");
    if (Object.getOwnPropertySymbols(current).length) throw new CanonicalJsonError("canonical JSON rejects symbol keys");
    if (ancestors.has(current)) throw new CanonicalJsonError("canonical JSON rejects circular values");
    ancestors.add(current);
    try {
      if (Array.isArray(current)) {
        for (let index = 0; index < current.length; index += 1) if (!Object.hasOwn(current, index)) throw new CanonicalJsonError("canonical JSON rejects sparse arrays");
...
export function canonicalJson(value) {
  try {
    ...
    if (error instanceof CanonicalJsonError) throw new ContractValidationError([issue("value", error.rule, error.message)]);
    throw error;
  }
}
...
export function personalNamePolicy(text) {
  ...
  const titleCase = words.length >= 3 && new RegExp(`^${nameWord}(?=$|[^\\p{L}])`, "u").test(policyText);
